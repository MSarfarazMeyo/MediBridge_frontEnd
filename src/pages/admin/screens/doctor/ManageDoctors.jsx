...
const ManageDoctors = () => {
  ...
  const {
    ...
    dataQueryFn: () =>
      getAllUsers(userState.userInfo.token, searchKeyword, currentPage, role),
    dataQueryKey: "users",
  ...
  return (
    <DataTable
      ...
      dataListName="Doctors"
      searchInputPlaceHolder="doctors's email..."
      searchKeywordOnSubmitHandler={submitSearchKeywordHandler}
      searchKeywordOnChangeHandler={searchKeywordHandler}
      searchKeyword={searchKeyword}
      tableHeaderTitleList={["Name", "Email", "Created At", "Actions"]}
      isLoading={isLoading}
      ...
      {usersData?.data.map((user) => (
        <tr key={user._id}>
          ...
          <td className="space-x-5 border-b border-gray-200 bg-white px-5 py-5 text-sm">
            <span
              onClick={() => {
                navigate(`/admin/doctor/profile`, { state: { id: user?._id } });
              }}
